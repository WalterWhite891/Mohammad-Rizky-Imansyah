...
class FormLogin:
    ...
    def aturKomponen(self):
        mainFrame = Frame(self.parent, bd=20,)
        ...
        # Label
        Label(mainFrame, text='Email:').grid(row=0, column=0, sticky=W, padx=10, pady=10)
        self.txtEmail = Entry(mainFrame ) 
        self.txtEmail.grid(row=0, column=1, padx=10, pady=10) 
        

        Label(mainFrame, text='Password:').grid(row=1, column=0, sticky=W, padx=10, pady=10)
...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    aplikasi = FormLogin(root, "Aplikasi Parkir",update_main_window)
    root.mainloop()